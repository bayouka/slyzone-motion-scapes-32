...
function schema(){return {type:'object',additionalProperties:false,properties:{directions:{type:'array',minItems:1,maxItems:MAX_DIRECTIONS,items:{type:'object',additionalProperties:false,properties:{name:{type:'string',maxLength:80},palette_id:{type:'string',enum:Object.keys(PALETTES)},typography_id:{type:'string',enum:Object.keys(TYPOGRAPHY)},shape_id:{type:'string',enum:Object.keys(SHAPES)},density_id:{type:'string',enum:Object.keys(DENSITIES)},imagery:{type:'string',enum:IMAGERY},motion:{type:'string',enum:MOTION},perception:{type:'array',maxItems:3,items:{type:'string',maxLength:50}},rationale:{type:'string',maxLength:420},dos:{type:'array',maxItems:4,items:{type:'string',maxLength:180}},donts:{type:'array',maxItems:4,items:{type:'string',maxLength:180}}},required:['name','palette_id','typography_id','shape_id','density_id','imagery','motion','perception','rationale','dos','donts']}}},required:['directions']}}
...
export async function handleLab2IdeaDesign(request,env){
  ...
  const directions=safeArray(payload.directions).slice(0,MAX_DIRECTIONS).map(resolveDirection).filter(Boolean);if(!directions.length)return json({ok:false,error:'AI_OUTPUT_INVALID'},502);
  return json({ok:true,contract_version:CONTRACT_VERSION,model:LAB2_MODEL,user_id:auth.id,directions,usage:usage(raw),catalog_version:'lab2-design-catalog-v1',guarantees:{single_ai_call:true,catalog_only_tokens:true,no_competitor_style_copy:true,human_direction_selection_required:true,deterministic_tokens_for_mockups:true,max_directions:MAX_DIRECTIONS}});
}
