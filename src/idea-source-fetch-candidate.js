...
function ipv4IsPublic(parts){
  ...
  // Documentation / benchmark ranges are not valid evidence origins.
...
function ipv6IsPublic(raw){
  const host=stripIpv6Brackets(lower(raw)).split('%')[0];
...
export function validatePublicSourceUrl(value){
  ...
  const host=stripIpv6Brackets(lower(url.hostname));
...
export function assertPublicResolvedAddresses(addresses){
  ...
  for(const raw of addresses){
    const value=stripIpv6Brackets(lower(raw));
...
export async function resolvePublicAddresses(hostname,fetchImpl=fetch){
  const host=stripIpv6Brackets(lower(hostname));
  ...
  for(const type of ['A','AAAA']){
    ...
    const r=await fetchImpl(u,{headers:{accept:'application/dns-json'},redirect:'error'});
...
export async function fetchCanonicalPublicSource(locator,{fetchImpl=fetch,resolveAddresses=resolvePublicAddresses,maxRedirects=MAX_REDIRECTS}={}){
  ...
  for(let hop=0;hop<=maxRedirects;hop++){
    ...
    const response=await fetchImpl(url,{method:'GET',redirect:'manual',headers:{accept:'text/html,text/plain,application/xhtml+xml;q=0.9','user-agent':'2b2c-source-fetch/0.1'}});
...
export const G2_SOURCE_FETCH_LIMITS=Object.freeze({max_redirects:MAX_REDIRECTS,max_body_bytes:MAX_BODY_BYTES,max_text_bytes:MAX_TEXT_BYTES});
