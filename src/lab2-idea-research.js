...
class Lab2ResearchError extends Error{
...
async function braveSearch(env,query){
  if(!env?.LAB2_BRAVE_SEARCH_API_KEY)return {ok:false,error:'SEARCH_UNCONFIGURED',results:[]};
  ...
  let response;
  try{
    response=await fetch(url.toString(),{headers:{accept:'application/json','x-subscription-token':String(env.LAB2_BRAVE_SEARCH_API_KEY)}});
  }catch{return {ok:false,error:'SEARCH_NETWORK_ERROR',results:[]}}
  if(!response.ok)return {ok:false,error:response.status===429?'SEARCH_QUOTA':'SEARCH_ERROR',results:[]};
  const payload=await response.json().catch(()=>null);
  const results=safeArray(payload?.web?.results).slice(0,6).map(item=>({
...
function dedupeCandidates(results,referenceHosts){
  const seen=new Set(referenceHosts);
  const out=[];
  for(const item of results){
    const host=hostOf(item.url);
    if(!host||seen.has(host)||GENERIC_HOSTS.has(host))continue;
    seen.add(host);
...
function selectionSchema(candidateCount){
  const indices=Array.from({length:candidateCount},(_,index)=>index);
  return {type:'object',additionalProperties:false,properties:{selected:{type:'array',maxItems:MAX_SELECTED_COMPETITORS,items:{type:'object',additionalProperties:false,properties:{candidate_index:{type:'integer',enum:indices},relation:{type:'string',enum:['DIRECT','NEAR','ALTERNATIVE']},reason:{type:'string',maxLength:300},confidence:{type:'string',enum:['HIGH','MEDIUM','LOW']}},required:['candidate_index','relation','reason','confidence']}}},required:['selected']};
}

function parseAi(raw){
  let payload=raw?.response??raw;
  if(typeof payload==='string'){try{payload=JSON.parse(payload)}catch{throw new Lab2ResearchError(502,'AI_OUTPUT_INVALID')}}
  if(!payload||typeof payload!=='object'||Array.isArray(payload))throw new Lab2ResearchError(502,'AI_OUTPUT_INVALID');
  return payload;
}

function usage(raw){
  const value=raw?.usage||raw?.meta?.usage||raw?.result?.usage||null;
  const input=Number(value?.prompt_tokens??value?.input_tokens??0),output=Number(value?.completion_tokens??value?.output_tokens??0);
  if(!Number.isFinite(input)||!Number.isFinite(output)||input<0||output<0||(!input&&!output))return null;
  return {prompt_tokens:Math.round(input),completion_tokens:Math.round(output),total_tokens:Math.round(input+output)};
}
...
async function selectCompetitors(env,input,candidates){
  ...
  }catch(error){const message=String(error?.message||error||'');throw new Lab2ResearchError(/quota|limit|capacity|neuron|rate/i.test(message)?429:502,/quota|limit|capacity|neuron|rate/i.test(message)?'AI_CAPACITY':'AI_ERROR')}
  const payload=parseAi(raw);
  const selected=safeArray(payload.selected).slice(0,MAX_SELECTED_COMPETITORS).map(item=>{
    const candidate=candidates[Number(item?.candidate_index)];
    if(!candidate)return null;
    return {...candidate,relation:['DIRECT','NEAR','ALTERNATIVE'].includes(item?.relation)?item.relation:'NEAR',selection_reason:cleanText(item?.reason,300),selection_confidence:['HIGH','MEDIUM','LOW'].includes(item?.confidence)?item.confidence:'LOW'};
  }).filter(Boolean);
...
async function analyzeSources(env,input,sources){
  ...
  const payload=parseAi(raw);
  const sourceAnalyses=safeArray(payload.sources).map(entry=>{
    const index=Number(entry?.source_index),source=observed[index];
    if(!source)return null;
    const findings=safeArray(entry.findings).map(finding=>validateFinding(source,finding)).filter(Boolean);
    return {source_index:index,url:source.url,role:source.role,findings};
  }).filter(Boolean);
...
export async function handleLab2IdeaResearch(request,env){
  ...
  let selection={selected:[],usage:null};
  try{selection=await selectCompetitors(env,input,candidatePool)}catch(error){return json({ok:false,error:error.code||'AI_ERROR'},error.status||502)}

  const sourceSeeds=[];
  input.references.forEach(reference=>sourceSeeds.push({role:'USER_REFERENCE',url:reference.url,reason:reference.reason,note:reference.note}));
  selection.selected.forEach(candidate=>sourceSeeds.push({role:'DISCOVERED_COMPETITOR',url:candidate.url,relation:candidate.relation,selection_reason:candidate.selection_reason,selection_confidence:candidate.selection_confidence}));
  const unique=[];const seen=new Set();
  for(const source of sourceSeeds){const key=source.url;if(!key||seen.has(key))continue;seen.add(key);unique.push(source);if(unique.length>=MAX_FETCHED_SOURCES)break}
  ...
  const enriched=joinAnalysis(fetched,analysis);
  const references=enriched.filter(item=>item.role==='USER_REFERENCE');
