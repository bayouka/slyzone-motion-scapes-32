const LAB2_MODEL='@cf/meta/llama-3.3-70b-instruct-fp8-fast';
...
function parseAi(raw){let payload=raw?.response??raw;if(typeof payload==='string'){const first=payload.indexOf('{'),last=payload.lastIndexOf('}');const candidate=first>=0&&last>first?payload.slice(first,last+1):payload;try{payload=JSON.parse(candidate)}catch{throw new Lab2ImproveError(502,'AI_OUTPUT_INVALID')}}if(!payload||typeof payload!=='object'||Array.isArray(payload))throw new Lab2ImproveError(502,'AI_OUTPUT_INVALID');return payload}
...
export async function handleLab2IdeaImprovements(request,env){
  ...
  ],response_format:{type:'json_schema',json_schema:schema()},temperature:0.2,max_completion_tokens:1800})}catch(error){const msg=String(error?.message||error||'');const capacity=/quota|limit|capacity|neuron|rate/i.test(msg);const structured=/json mode|schema|structured|invalid json/i.test(msg);return json({ok:false,error:capacity?'AI_CAPACITY':structured?'AI_OUTPUT_INVALID':'AI_ERROR'},capacity?429:502)}
