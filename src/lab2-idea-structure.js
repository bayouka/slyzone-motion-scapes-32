...
const CONTRACT_VERSION='lab2-structure-v3';
const DEFINITION_CONTRACT='lab2-definition-v1';
const FEASIBILITY_CONTRACT='lab2-feasibility-v1';
...
function normalizeDefinition(v){
  ...
  return {
    ...
    brief:{one_liner:cleanText(brief.one_liner,320),problem:cleanText(brief.problem,900),target_users:safeArray(brief.target_users).slice(0,4).map(x=>cleanText(x,180)).filter(Boolean),solution:cleanText(brief.solution,1200),core_features:safeArray(brief.core_features).slice(0,10).map(x=>cleanText(x,700)).filter(Boolean),main_flow:safeArray(brief.main_flow).slice(0,8).map(x=>cleanText(x,700)).filter(Boolean),differentiators:safeArray(brief.differentiators).slice(0,6).map(x=>cleanText(x,700)).filter(Boolean)},
...
function normalizeFeasibility(v){
  if(!v||typeof v!=='object')return null;
  const resolved=v.resolved_decisions&&typeof v.resolved_decisions==='object'&&!Array.isArray(v.resolved_decisions)?v.resolved_decisions:{};
  return {
    level:cleanText(v.level,20),simple:v.simple===true,summary:cleanText(v.summary,500),
    capabilities:safeArray(v.capabilities).slice(0,16).map(x=>({id:cleanText(x?.id,50),label:cleanText(x?.label,140),complexity:cleanText(x?.complexity,20),external_dependency:x?.external_dependency===true,rationale:cleanText(x?.rationale,420)})).filter(x=>x.id),
    open_decisions:safeArray(v.open_decisions).slice(0,10).map(x=>({question:cleanText(x?.question,320),blocking:x?.blocking===true,reason:cleanText(x?.reason,320)})).filter(x=>x.question),
    resolved_decisions:Object.fromEntries(Object.entries(resolved).map(([question,answer])=>[cleanText(question,320),cleanText(answer,1200)]).filter(([q,a])=>q&&a))
  };
}
function assertFeasibilityResolved(feasibility){for(const item of feasibility.open_decisions){if(item.blocking&&!cleanText(feasibility.resolved_decisions[item.question],1200))throw new Lab2StructureError(409,'FEASIBILITY_DECISIONS_INCOMPLETE')}}
function normalizeInput(body){
  if(!body||typeof body!=='object'||Array.isArray(body))throw new Lab2StructureError(400,'INVALID_REQUEST');
  if(cleanText(body.definition_contract,60)!==DEFINITION_CONTRACT)throw new Lab2StructureError(400,'DEFINITION_INCOMPATIBLE');
  if(cleanText(body.feasibility_contract,60)!==FEASIBILITY_CONTRACT)throw new Lab2StructureError(400,'FEASIBILITY_INCOMPATIBLE');
  const definition=normalizeDefinition(body.definition),feasibility=normalizeFeasibility(body.feasibility);
  if(!definition?.project_name||!definition?.brief?.one_liner||!definition?.brief?.problem||!definition?.brief?.solution)throw new Lab2StructureError(400,'DEFINITION_REQUIRED');
  if(!feasibility)throw new Lab2StructureError(400,'FEASIBILITY_REQUIRED');
  assertFeasibilityResolved(feasibility);
  return {definition,feasibility};
}
...
export async function handleLab2IdeaStructure(request,env){
  ...
  const d=input.definition,b=d.brief,f=input.feasibility,structural=d.open_decisions.filter(x=>x.scope==='STRUCTURE');
  const capabilityText=f.capabilities.length?f.capabilities.map(x=>`${x.id}: ${x.label} (${x.complexity})`).join(' | '):'aucune capacité technique complexe détectée';
  const resolvedText=Object.entries(f.resolved_decisions).map(([question,answer])=>`Q: ${question}\nR: ${answer}`).join('\n')||'aucune décision de faisabilité ajoutée';
  let raw;try{raw=await env.AI.run(LAB2_MODEL,{messages:[
    {role:'system',content:'Tu transformes une définition canonique déjà validée et une analyse de faisabilité confirmée en EXPERIENCE ARCHITECTURE provisoire. Tu n’inventes aucune nouvelle capacité métier. Tu adaptes l’architecture au project_profile : un site de service peut n’avoir que PUBLIC/UTILITY ; un SaaS peut avoir AUTH/APP/SETTINGS ; une marketplace peut avoir plusieurs parcours acteurs ; ADMIN uniquement si le contexte l’exige réellement. Les capacités détectées par la faisabilité indiquent des contraintes techniques à respecter, pas des fonctionnalités à ajouter. Les décisions de faisabilité résolues sont des décisions humaines et doivent être respectées. Une page ou écran n’existe que s’il a un rôle distinct. Le nombre minimal cohérent est préférable. Les décisions STRUCTURE non résolues ne doivent pas être tranchées silencieusement : place-les dans structural_questions si elles conditionnent encore l’architecture. Les workflows décrivent les parcours humains essentiels avec des verbes simples. Réponds strictement selon le schéma JSON.'},
    {role:'user',content:`Projet: ${d.project_name}\nProfil: ${JSON.stringify(d.project_profile)}\nConcept: ${b.one_liner}\nProblème: ${b.problem}\nPublics: ${b.target_users.join(', ')||'non précisés'}\nSolution: ${b.solution}\nCapacités validées: ${b.core_features.join(' | ')||'aucune capacité supplémentaire'}\nParcours validé: ${b.main_flow.join(' > ')}\nDifférenciation validée: ${b.differentiators.join(' | ')||'aucune'}\nDécisions STRUCTURE ouvertes: ${structural.map(x=>x.question).join(' | ')||'aucune'}\n\nFAISABILITÉ CONFIRMÉE\nNiveau: ${f.level||'LOW'}\nCapacités techniques détectées: ${capabilityText}\nDécisions humaines de faisabilité:\n${resolvedText}`}
