...
const source=await fs.readFile(new URL('../src/lab2-idea-research.js',import.meta.url),'utf8');
const moduleUrl=`data:text/javascript;base64,${Buffer.from(source).toString('base64')}`;
const {handleLab2IdeaResearch}=await import(moduleUrl);
...
function request(payload=body,token='good-token'){
  return new Request('https://app.example/api/lab2/research',{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${token}`},body:JSON.stringify(payload)});
}
...
if(disabled.status!==404)throw new Error('LAB2_RESEARCH_FLAG_GUARD_FAILED');
const forbidden=await handleLab2IdeaResearch(request(body,'other-token'),env);
if(forbidden.status!==403)throw new Error('LAB2_RESEARCH_ALLOWLIST_GUARD_FAILED');
const unconfigured=await handleLab2IdeaResearch(request(),{...env,LAB2_ALLOWED_USER_IDS:''});
