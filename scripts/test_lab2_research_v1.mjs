...
let braveCalls=0,tavilyCalls=0;
globalThis.fetch=async(url,options={})=>{
  ...
  if(target==='https://api.tavily.com/search'){tavilyCalls+=1;return Response.json({results:[{title:'Fallback competitor',url:'https://fallback.example/',content:'Solution proche du besoin.'}]})}
...
const ai={run:async()=>{
  aiCalls+=1;
  if(aiCalls===1)return {response:{selected:[
    ...
    {candidate_index:1,relation:'ADJACENT',reason:'Le workflow de tiers et compte rendu paraît proche.',confidence:'MEDIUM'}
  ]},usage:{prompt_tokens:500,completion_tokens:120}};
  return {response:{sources:[
    {source_index:0,findings:[{category:'WORKFLOW',statement:'La référence permet de créer une demande puis de recevoir un rapport.',support_text:'crée une demande et reçoit un rapport'},{category:'FUNCTIONALITY',statement:'Ce constat doit être supprimé car il n’est pas sourcé.',support_text:'texte absent de la source'}]},
    {source_index:1,findings:[{category:'POSITIONING',statement:'La solution se présente comme une inspection avant déplacement.',support_text:'inspecter un véhicule à distance avant déplacement'}]},
  ...
  ],cross_patterns:[{statement:'Les solutions observées mettent en avant une vérification à distance suivie d’un retour structuré.',source_indices:[0,1,2]}],limitations:['Les pages publiques ne permettent pas de confirmer les étapes après connexion.']},usage:{prompt_tokens:1600,completion_tokens:430}};
}};
...
const env={LAB2_IDEA_STUDIO_ENABLED:'true',LAB2_RESEARCH_ENABLED:'true',LAB2_ALLOWED_USER_IDS:'11111111-1111-4111-8111-111111111111',LAB2_BRAVE_SEARCH_API_KEY:'test-key',SUPABASE_URL:'https://example.supabase.co',SUPABASE_PUBLISHABLE_KEY:'pub',AI:ai,SOURCE_FETCH:sourceFetch};
const body={
  name:'TierceVue',
  understanding:{
    contract_version:'lab2-understanding-v4',
    one_liner:'Une plateforme pour faire vérifier une voiture éloignée avant de se déplacer.',
    ...
    target_users:[{label:'Acheteurs de véhicules à distance'}],main_flow:[{step:'Repérer une voiture'},{step:'Demander une vérification'}],
    deferred_questions:[{question:'La vérification est-elle rémunérée ?',scope:'FEASIBILITY'}]
  },
  references:[{url:'ref.example',reason:'parcours',note:'J’aime la logique de demande et rapport.'}],discover_solutions:true
};
...
const response=await handleLab2IdeaResearch(request(),env),result=await response.json();
if(response.status!==200||result.ok!==true)throw new Error('LAB2_RESEARCH_SUCCESS_EXPECTED');
if(result.contract_version!=='lab2-research-v3')throw new Error('LAB2_RESEARCH_V3_REQUIRED');
if(result.research_plan?.profile?.model!=='MARKETPLACE'||!result.research_plan?.candidate_types?.includes('SUBSTITUTE'))throw new Error('LAB2_ADAPTIVE_RESEARCH_PLAN_FAILED');
...
if(result.solutions?.length!==2||result.solutions?.[1]?.relation!=='ADJACENT')throw new Error('LAB2_SOLUTION_SELECTION_MISMATCH');
...
console.log(`lab2-research-v3: ok (${braveCalls} bounded searches, ${aiCalls} mocked AI calls; adaptive plan verified)`);
