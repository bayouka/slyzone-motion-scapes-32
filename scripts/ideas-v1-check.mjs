...
const assert=(ok,msg)=>{if(!ok)throw new Error(`IDEAS COMPATIBILITY CHECK FAILED: ${msg}`);};
const index=read('site/index.html');
const js=read('site/assets/ideas-v1.js');
const css=read('site/assets/ideas-v1.css');
const bridge=read('site/assets/ideas-canonical-bridge-v1.js');
...
const legacyDisable=read('supabase/migrations/20260916193059_disable_legacy_idea_decision_conversion_v1.sql');

for(const marker of ['ideas-v1.css?v=1.0.0',"ideas-v1.js?v=1.0.0",'ideas-canonical-bridge-v1.js?v=1.0.0']) assert(index.includes(marker),`index missing ${marker}`);
assert(!index.includes('ideas-final-decision-v1.js'),'obsolete final-decision module must not be bootstrapped');

for(const marker of [
  '__4B4C_IDEAS_V1__','create_idea_v1','update_idea_core_v1','set_idea_access_v1','create_idea_item_v1',
  'vote_idea_item_v1','submit_idea_review_v1','#/ideas/','Idées en réflexion','4b4c vous guide','Mode présentation'
]) assert(js.includes(marker),`legacy compatibility runtime missing ${marker}`);

for(const marker of [
  'workspace-v3','data-idea-action','decide','convert','Continuer dans le workspace canonique'
]) assert(bridge.includes(marker),`canonical compatibility bridge missing ${marker}`);
...
for(const marker of [
  ...
  'app_private.can_write_idea','app_private.rpc_create_project_with_access_v1'
]) assert(migration.includes(marker),`historical migration missing ${marker}`);
...
]) assert(legacyDisable.includes(marker),`legacy decision/conversion hardening missing ${marker}`);

assert(!js.includes("api.insert('projects'"),'legacy UI must never create projects directly');
...
console.log('Ideas compatibility gate: OK — legacy rendering retained, decision/promotion authority canonicalized');
