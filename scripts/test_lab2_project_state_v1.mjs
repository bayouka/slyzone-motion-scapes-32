...
await state.setArtifact('structure',{status:'CONFIRMED',contractVersion:'lab2-structure-v1',data:{pages:['/']}});
...
await state.setArtifact('structure',{status:'CONFIRMED',contractVersion:'lab2-structure-v1',data:{pages:['/','/contact']}});
let snapshot=state.read();
if(snapshot.artifacts.design.status==='STALE')throw new Error('LAB2_STRUCTURE_CHANGE_MUST_NOT_INVALIDATE_DESIGN');
if(snapshot.artifacts.mockups.status!=='STALE'||snapshot.artifacts.presentation.status!=='STALE')throw new Error('LAB2_STRUCTURE_DEPENDENCY_INVALIDATION_FAILED');
...
console.log('lab2-project-state-v1: ok (precise dependency invalidation verified)');
