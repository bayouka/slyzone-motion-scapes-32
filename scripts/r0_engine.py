...
def resolve_requirements(bp: dict, state: dict, contexts: set[str]) -> dict:
    ...
    for rid, req in bp["requirements"].items():
        ...
        rec = supplied.get(rid, {})
        levels = set(rec.get("levels", []))
        if not applicable:
            status = "NOT_RELEVANT"
        elif rec.get("stale"):
            status = "STALE"
        elif rec.get("conflicted"):
            status = "CONFLICTED"
        elif rec.get("accepted_unknown"):
        ...
        if role and any(level in levels for level in {"HUMAN_DECISION", "EXPERT_SIGNOFF"}):
            authority_ok = rec.get("authority_role") in {role, "USER"} or role in set(state.get("authority_roles", []))
...
def _req_satisfies(req: dict, rs: dict, gate_id: str, allow_unknown: bool) -> tuple[bool, bool]:
    if not rs["applicable"] or rs["status"] == "NOT_RELEVANT":
        return True, False
    if rs["status"] == "ACCEPTED_UNKNOWN":
        return allow_unknown, allow_unknown
    if rs["status"] != "RESOLVED" or not rs["authority_ok"]:
        return False, False
    return _level_ok(set(rs["levels"]), _minimum(req, gate_id)), False


def evaluate_gates(bp: dict, state: dict, req_states: dict) -> dict:
    out = {}
    ...
    for gate_id in bp["gate_order"]:
        ...
        atom_ids = (binding.get("required_atoms", []) or []) + (binding.get("conditional_atoms", []) or [])
        for rid in atom_ids:
            req = bp["requirements"].get(rid)
            rs = req_states.get(rid)
            if not req or not rs or not rs["applicable"]:
                continue
            if rs["status"] == "STALE":
                stale.append(rid)
                continue
            ok, unknown = _req_satisfies(req, rs, gate_id, allow_unknown)
            used_unknown |= unknown
            if not ok:
                missing.append(rid)

        for artifact_id in binding.get("required_artifacts", []) or []:
            artifact = artifacts.get(artifact_id, {})
            if not artifact.get("ready") or artifact.get("stale"):
...
def current_gate(bp: dict, gates: dict):
    for gate_id in bp["gate_order"]:
        if gates[gate_id]["status"] not in {"READY", "READY_WITH_ACCEPTED_UNKNOWNS"}:
            return gate_id
...
def _target_ids(bp: dict, gate_id: str) -> list[str]:
    binding = bp["bindings"].get(gate_id, {})
    return list(dict.fromkeys((binding.get("required_atoms", []) or []) + (binding.get("conditional_atoms", []) or [])))


def plan_actions(bp: dict, state: dict, req_states: dict, gates: dict) -> tuple[list[dict], dict | None]:
    gate_id = current_gate(bp, gates)
    actions = []
    human = []
    for rid in _target_ids(bp, gate_id):
        req = bp["requirements"].get(rid)
        rs = req_states.get(rid)
        if not req or not rs or not rs["applicable"] or rs["status"] in {"RESOLVED", "NOT_RELEVANT"}:
            continue
        ...
        if mode in FORMAL_HUMAN or human_only:
            ...
            continue
        auto = next((path for path in paths if path in AUTO_PATHS), None)
        if auto:
            actions.append({"requirement_id": rid, "path": auto, "gate": gate_id, "fingerprint": rs["fingerprint"]})
...
def project(bp: dict, state: dict) -> dict:
    ...
    gate_states = evaluate_gates(bp, state, requirement_states)
    actions, human = plan_actions(bp, state, requirement_states, gate_states)
    promotion = []
    if gate_states.get("G7_IDEA_APPROVAL_READY", {}).get("status") in {"READY", "READY_WITH_ACCEPTED_UNKNOWNS"}:
