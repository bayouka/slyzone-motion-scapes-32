...
const VERSION = '2b2c workflow backend bridge v2.0.0';
...
function notice(form, message, error = false) {
  let node = form.querySelector('[data-workflow-v2-feedback]');
  if (!node) {
    node = document.createElement('div');
    node.dataset.workflowV2Feedback = '1';
...
async function submitActionEdit(form, fd) {
  ...
  const blocked = val(fd,'blockedReason');
  ...
  await api.rpc('set_action_status_v1', { p_action_id: actionId, p_status: status, p_blocked_reason: status === 'blocked' ? blocked || null : null });
...
async function submitMeeting(form, fd) {
  ...
  const projectId = val(fd,'projectId') || null;
  const attendeeIds = vals(fd,'attendeeIds');
  const guestIds = new Set();
  // The server remains the source of truth for guest/project access. The UI only
  // requests shared visibility when a guest checkbox is explicitly marked.
  form.querySelectorAll('input[name="attendeeIds"][data-guest="1"]:checked').forEach(x=>guestIds.add(x.value));
  const visibility = guestIds.size ? 'shared' : (val(fd,'visibility','internal') || 'internal');
  ...
  await done(form,'Réunion planifiée avec ses participants.', meetingId ? `#/calendar/meeting/${meetingId}` : '#/calendar');
}
...
async function submitMemberManage(form, fd) {
  ...
  if (role === 'member') {
    const projects = await api.select('projects', `select=id,visibility&workspace_id=eq.${wid}&status=neq.archived`);
  ...
  await api.rpc('set_workspace_member_access_v1', { p_workspace_id: wid, p_user_id: userId, p_role: role, p_project_ids: projectIds });
...
async function submitProjectEdit(form, fd) {
  ...
  await api.update('projects', `id=eq.${projectId}`, {
    name: val(fd,'name'),
    objective: val(fd,'objective'),
    target_date: val(fd,'targetDate') || null,
...
document.addEventListener('submit', async (event) => {
  ...
  stop(event);
  if (form.dataset.workflowV2Busy === '1') return;
  form.dataset.workflowV2Busy = '1';
  setBusy(form,true);
...
document.addEventListener('change', async (event) => {
  ...
  if (!select) return;
  stop(event);
  const actionId = select.dataset.statusAction;
  ...
  try {
    await api.rpc('set_action_status_v1',{ p_action_id:actionId, p_status:status, p_blocked_reason:blockedReason });
...
// Remove misleading legacy controls from the member dialog. Project visibility
// belongs to the workspace role/access workflow; task/milestone responsibility
// is managed where the work actually lives.
function clarifyMemberDialog() {
  document.querySelectorAll('form[data-form="member-manage"]').forEach(form => {
    if (form.dataset.workflowV2Clarified === '1') return;
    form.dataset.workflowV2Clarified = '1';
    const responsibility = [...form.querySelectorAll('.member-project-grid > div')].find(x => /Responsabilité/i.test(x.textContent || ''));
    if (responsibility) {
      responsibility.innerHTML = '<span class="eyebrow">Responsabilités</span><div class="notice">Les responsables des actions et jalons se choisissent directement dans la roadmap. Le responsable principal du projet se transfère depuis les accès du projet.</div>';
    }
    const accessMode = form.elements.accessMode?.closest('.field');
    if (accessMode) accessMode.style.display = 'none';
  });
}
const observer = new MutationObserver(clarifyMemberDialog);
