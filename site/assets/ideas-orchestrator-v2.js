...
(()=>{
  ...
  async function patchDetail(){
    ...
    if(bar){bar.classList.add('ideas-v2-orchestrated');bar.innerHTML=detailBarHtml(s);}
    const guide=document.querySelector('.ideas-v1-guide');
    if(guide){
      const m=phaseMeta[s.phase]||phaseMeta.clarify;
      guide.classList.add('ideas-v2-guide');
      guide.innerHTML=`<div class="ideas-v2-guide-icon">✦</div><div><small>2b2c vous guide</small><h2>${esc(m.title)}</h2><p>${esc(s.phase==='clarify'&&s.missing.length?`Commencez par ${s.missing.slice(0,2).join(' et ')}. Les réponses seront enregistrées directement dans l’idée.`:m.text)}</p></div>`;
    }
    const maturity=document.querySelector('.ideas-v1-maturity-head');
    if(maturity){const m=phaseMeta[s.phase]||phaseMeta.clarify;maturity.innerHTML=`<div><small>Progression</small><strong>${esc(m.label)}</strong></div><span class="ideas-v2-stage-badge">${m.step}/5</span>`;}
    const checks=document.querySelector('.ideas-v1-checks');
    if(checks) checks.innerHTML=stages((phaseMeta[s.phase]||phaseMeta.clarify).step);
    document.querySelectorAll('.ideas-v1-hint').forEach(n=>n.remove());
    patchBranding(); patchSidebar();
  }

  async function projectsData(){
    const w=workspaceId();if(!w||!api.getSession()?.access_token)return[];
    return api.select('ideas',`select=id,title,summary,original_text,status,visibility,problem,audience,proposal,updated_at&workspace_id=eq.${w}&order=updated_at.desc`).catch(()=>[]);
  }

  function statusText(i){
    if(i.status==='approved') return 'Approuvée'; if(i.status==='in_review') return 'En discussion'; if(i.status==='ready_for_review') return 'Prête à partager'; if(i.status==='exploring') return 'En exploration'; if(i.status==='needs_work') return 'À approfondir'; return 'À clarifier';
  }
  ...
  async function patchProjects(){
    ...
    document.querySelectorAll('.ideas-v1-projects-panel,.ideas-v2-projects-hub').forEach(n=>n.remove());
    ...
    const hub=document.createElement('section');hub.className='ideas-v2-projects-hub';
    ...
    const anchor=heading?.closest('header,section,div');
    if(anchor?.parentNode) anchor.parentNode.insertBefore(hub,anchor.nextSibling); else content?.prepend(hub);
    patchSidebar(); patchBranding();
  }

  function patchSidebar(){
    if(!ideaId()&&route()!=='#/ideas'&&!route().startsWith('#/ideas/'))return;
    document.querySelectorAll('a[href="#/"],a[href="#/dashboard"]').forEach(a=>a.classList.remove('active','selected'));
    document.querySelectorAll('a[href="#/projects"]').forEach(a=>a.classList.add('active'));
  }

  function patchBranding(){
    if(!ideaId()&&route()!=='#/projects'&&!route().startsWith('#/ideas'))return;
    document.querySelectorAll('.ideas-v1-modal input[placeholder],.ideas-ai-v1-panel small,.ideas-v1-guide small').forEach(n=>{if(n.placeholder)n.placeholder=n.placeholder.replace(/4b4c/gi,'2b2c');if(n.textContent)n.textContent=n.textContent.replace(/4b4c/gi,'2b2c')});
  }

  async function doAction(action){
    if(['understand','questions','improve','challenge','synthesize'].includes(action)) return window.__4B4C_IDEAS_AI_V1__?.run?.(action);
    if(action==='evidence') return window.__4B4C_IDEAS_EVIDENCE_V1__?.openResearch?.();
    if(action==='presentation') return window.__4B4C_IDEAS_EVIDENCE_V1__?.presentation?.();
    if(action==='share') return document.querySelector('[data-idea-action="share"]')?.click();
    if(action==='decision'){const id=ideaId();if(id)location.hash=`#/ideas/${id}/decision`;return;}
    if(action==='convert') return document.querySelector('[data-idea-action="convert"]')?.click();
    if(action==='projects')location.hash='#/projects';
  }
  ...
  window.addEventListener('hashchange',refresh);window.addEventListener('4b4c:ideas-refresh',refresh);
  new MutationObserver(refresh).observe(document.documentElement,{childList:true,subtree:true});
  setTimeout(refresh,600);
  window.__2B2C_IDEAS_ORCHESTRATOR_V2__=Object.freeze({version:'2.0.0-progressive',refresh});
})();
