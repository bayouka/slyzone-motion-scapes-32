...
(() => {
  ...
  function status(member,ownId){
    if(member.user_id===ownId)return {tone:'online',label:'Vous êtes en ligne'};
  ...
  function memberCard(member,profile,ownId){
    const s=status(member,ownId);const own=member.user_id===ownId;const name=profile?.display_name||(own?'Vous':'Membre');
    const avatar=profile?.avatar_url?`<img src="${esc(profile.avatar_url)}" alt="">`:`<span>${esc(initials(name))}</span>`;
    return `<article class="home-team-v2-person"><div class="home-team-v2-avatar">${avatar}<i class="${s.tone}"></i></div><div class="home-team-v2-meta"><strong>${esc(name)}</strong><small>${esc(s.label)}</small></div>${own?'':`<div class="home-team-v2-actions"><a href="#/messages" aria-label="Envoyer un message à ${esc(name)}" title="Message">${messageIcon()}</a><button type="button" data-home-team-v2-call="${esc(member.user_id)}" aria-label="Appeler ${esc(name)}" title="Appeler">${callIcon()}</button></div>`}</article>`;
  }
  ...
  function render(team,map,ownId){
    ...
    const online=team.filter((m)=>status(m,ownId).tone==='online').length;
    section.innerHTML=`<div class="home-team-v2-head"><div><strong>Collaborateurs</strong><span><i></i>${online} en ligne</span></div><a href="#/team">Équipe</a></div><div class="home-team-v2-list">${team.map((m)=>memberCard(m,map.get(m.user_id),ownId)).join('')}</div>`;
  }
  ...
  window.__4B4C_HOME_TEAM_V2__=Object.freeze({version:'2.0.0',refresh:load});
})();
