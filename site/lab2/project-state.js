(() => {
  ...
  const invalidateAfterInState=(state,stage,reason='upstream_changed')=>{
    const start=STAGES.indexOf(stage);if(start<0)return state;
    for(let index=start+1;index<STAGES.length;index+=1){
      const name=STAGES[index];const current=normalizeArtifact(name,state.artifacts[name]);
  ...
  const setArtifact=async(stage,{status='READY',contractVersion=null,inputFingerprint=null,data=null,provenance=[]}={})=>{
    ...
    const state=read();const previous=normalizeArtifact(stage,state.artifacts[stage]);
    ...
    write(state);return state.artifacts[stage];
  };
  ...
  window.Lab2ProjectState=Object.freeze({STORAGE_KEY,VERSION,STAGES:[...STAGES],read,write,hash,getArtifact,setArtifact,confirmArtifact,setRawIdea,markStatus,invalidateAfter,clear});
})();