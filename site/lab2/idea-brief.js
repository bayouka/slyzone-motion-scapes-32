(() => {
  ...
  function decisionStats(){
    ...
    return {
      accepted:values.filter(x=>x?.status==='ACCEPTED').length,
      modified:values.filter(x=>x?.status==='MODIFIED').length,
  ...
  function decisionsComplete(){
    ...
    return proposals.length===0||proposals.every(p=>['ACCEPTED','MODIFIED','REJECTED'].includes(decisions?.[p.id]?.status));
  }
  function renderDecisionSummary(){
    decisionSummary.innerHTML='';const stats=decisionStats();
    for(const [value,label] of [[stats.accepted,'acceptée(s)'],[stats.modified,'modifiée(s)'],[stats.rejected,'refusée(s)']]){
      const item=document.createElement('span');item.textContent=`${value} ${label}`;decisionSummary.appendChild(item);
    }
  }
  function renderList(node,items,emptyText,ordered=false){
  ...
  function renderRetained(items){
    ...
    values.forEach(item=>{
      const card=document.createElement('div');card.className='retained-item';
      const strong=document.createElement('strong');strong.textContent=item.title||'Amélioration retenue';
      const span=document.createElement('span');span.textContent=item.text||'';
      card.append(strong,span);retainedList.appendChild(card);
    });
  }
  function render(payload){
    ...
    renderList(briefFlow,brief.main_flow,'Workflow encore à préciser.',true);
    ...
    renderRetained(payload?.retained_improvements);
    briefSection.hidden=false;errorBox.hidden=true;loadingBox.hidden=true;
  }
  function save(payload){
    try{localStorage.setItem(BRIEF_KEY,JSON.stringify({version:1,fingerprint:fingerprintValue,response:payload,savedAt:new Date().toISOString()}))}catch{}
  }
  ...
  async function loadContext(){
    ...
    const baseReady=Boolean(draft?.name&&draft?.description&&understanding?.one_liner&&understanding?.problem&&improvements?.version===1&&Array.isArray(improvements?.proposals));
    const ready=baseReady&&decisionsComplete();
    ...
    fingerprintValue=await fingerprint({name:draft.name,description:draft.description,references:draft.references||[],understanding,proposals:improvements.proposals,decisions:improvements.decisions||{}});
    const cached=parse(localStorage.getItem(BRIEF_KEY));
    if(cached?.version===1&&cached?.fingerprint===fingerprintValue&&cached?.response?.ok)render(cached.response);
  }

  async function callApi(){
    const access=token();if(!access)throw Object.assign(new Error('UNAUTHORIZED'),{code:'UNAUTHORIZED',status:401});
    const response=await fetch('/api/lab2/brief',{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${access}`},body:JSON.stringify({
      name:draft.name,original_description:draft.description,references:Array.isArray(draft.references)?draft.references:[],understanding,
      proposals:improvements.proposals,decisions:improvements.decisions||{}
    })});
    const payload=await response.json().catch(()=>({}));
    if(!response.ok||!payload?.ok)throw Object.assign(new Error(payload?.error||'BRIEF_ERROR'),{code:payload?.error||'BRIEF_ERROR',status:response.status});
    return payload;
  }
  function errorLabel(error){
    ...
    if(error?.code==='UNAUTHORIZED')return 'Ta session 4b4c a expiré. Reconnecte-toi puis réessaie.';
  ...
  generateButton.addEventListener('click',async()=>{
    ...
    try{const payload=await callApi();save(payload);render(payload)}
    catch(error){loadingBox.hidden=true;errorBox.hidden=false;errorBox.textContent=errorLabel(error)}
