...
4. Inspect the current implementation before changing it. Existing production code may contain historical behavior that is no longer the conceptual target.
...
- `docs/idea-engine/canonical/CAPTURE_INGESTION_MEMORY_CONTRACT_V1.md`
...
- Dated audits, prototypes, test reports and walkthroughs are supporting evidence unless explicitly marked canonical.
...
2. **2b2c should do more work than the user.** Before asking a question, check memory, raw brief, attached sources, existing answers, researchability and reasonable inference.
...
4. **RAW FIRST.** Persist the Idea, raw brief, links and file/source references before depending on any LLM call.
5. **`Commencer` is not `Sauvegarder`.** Autosave happens before it. `Commencer` flushes pending writes/uploads, snapshots the capture version, starts analysis and allows navigation to the workspace.
...
8. **Extraction is atomic and multi-domain.** One sentence may populate audience, constraints, features, visual preferences, decision requirements, etc.
9. **Understand intent, negation, modality and time.** `I hate red`, `my old site was red`, `red could work`, and `I want red` are different facts/states.
10. **Prefill does not freeze.** Early information can prepopulate a later domain without becoming a final decision.
11. **Visibility is not existence.** Information may exist in memory long before the corresponding UX surface becomes relevant.
12. **Provenance is mandatory.** Human-declared facts, source-extracted observations, web research, system calculations, AI inference and AI recommendations must remain distinguishable.
13. **AI inference never silently becomes human truth.** Ambiguity remains ambiguity until legitimately resolved.
14. **Recommendations must be explainable.** A valid outcome is `insufficient information`; never fabricate confidence or force a recommendation.
15. **Readiness is relative to the decision.** Decision Requirements define what this person/team needs before deciding; do not use arbitrary completion percentages.
16. **Change Intelligence is targeted.** Re-evaluate only affected dependencies after cosmetic/contextual/substantive/critical changes; do not restart the whole journey.
17. **Presentation and workshop are optional.** Solo and team decisions are branches of the same engine.
18. **GO is not privileged.** Deepen, pause and stop are valid successful outcomes.
19. **Project Draft is created only after an explicit launch decision.** Transfer active useful context selectively; rejected/superseded material remains historical.
20. **The LLM is not the sole state engine.** Permissions, persistence, provenance, deterministic dependencies, important mutations and stale-safety require deterministic application/database rules.
...
3. update `KNOWLEDGE.md` only if the knowledge map/version/path changes;
