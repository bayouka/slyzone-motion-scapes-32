...
def req(rid, *, default="ACTIVE", anyctx=None, paths=None, minimum=None, human=None, human_only=None, deps=None, impact=None, authority=None):
    item = {
        ...
        "resolution": {
            "allowed_classes": ["R1"],
            "preferred_paths": paths or ["AI_R"],
            "default_human_interaction": human or "NONE",
        },
    }
    if anyctx:
...
def blueprint():
    ...
    gates = [
        "G0_BLUEPRINT_FIT_SUFFICIENT",
        "G1_FOUNDATION_LOCKABLE",
        "G2_EVIDENCE_CONTEXT_SUFFICIENT",
        "G7_IDEA_APPROVAL_READY",
    ...
    bindings = {
        "G0_BLUEPRINT_FIT_SUFFICIENT": {
            "system_conditions": ["RAW_IDEA_PERSISTED", "PROVENANCE_PERSISTED", "BLUEPRINT_CLASSIFICATION_AVAILABLE"],
            "required_atoms": ["SV.D04.CREATION_OR_REDESIGN"],
        },
        "G1_FOUNDATION_LOCKABLE": {
            "required_atoms": ["SV.D02.ORG_CONTEXT", "SV.D02.DECLARED_PROBLEM", "SV.D02.PRIMARY_OBJECTIVE", "SV.D02.USER_OUTCOME", "SV.D03.PRIMARY_AUDIENCE", "SV.D04.OFFER_BASELINE"]
        },
        "G2_EVIDENCE_CONTEXT_SUFFICIENT": {
            "required_atoms": ["SV.D03.PRIMARY_NEED", "SV.D05.MARKET_CONTEXT", "SV.D05.COMPETITOR_SET", "SV.D05.PATTERN_GAP_SYNTHESIS", "SV.D05.RESEARCH_SUFFICIENCY"]
        },
    ...
    return {
        "manifest": {"blueprint_version": "0.3"},
...
class R0Tests(unittest.TestCase):
    ...
    def base(self):
        return {
            "facts": {"project_type": "SITE_VITRINE", "idea": {"creation_or_redesign": "creation"}},
            "system_conditions": ["RAW_IDEA_PERSISTED", "PROVENANCE_PERSISTED", "BLUEPRINT_CLASSIFICATION_AVAILABLE"],
            "requirements": {"SV.D04.CREATION_OR_REDESIGN": evidence("ACCEPTED_AS_CURRENT")},
        }
    ...
    def fill_g1(self, state):
        state["requirements"].update({
            ...
            "SV.D02.USER_OUTCOME": evidence("WORKING_ASSUMPTION"),
            "SV.D03.PRIMARY_AUDIENCE": evidence("WORKING_ASSUMPTION"),
            "SV.D04.OFFER_BASELINE": evidence("RAW_HUMAN"),
        })

    def fill_g2(self, state):
        state["requirements"].update({
            "SV.D03.PRIMARY_NEED": evidence("WORKING_ASSUMPTION"),
            "SV.D05.MARKET_CONTEXT": evidence("CALCULATED"),
            "SV.D05.COMPETITOR_SET": evidence("SOURCE_BACKED"),
            "SV.D05.PATTERN_GAP_SYNTHESIS": evidence("AI_RECOMMENDATION"),
    ...
    def test_expert_signoff_context_activates_requirement(self):
        state = self.base()
        self.fill_g1(state)
        self.fill_g2(state)
        state["facts"]["has_regulatory_constraint"] = True
        projection = e.project(self.bp, state)
    ...
    def test_accepted_unknown_can_be_forbidden(self):
        ...
        self.fill_g1(state)
        self.fill_g2(state)
        state["accepted_unknown_allowed"] = False
        state["requirements"]["SV.D05.COMPETITOR_SET"] = {"accepted_unknown": True}
        projection = e.project(self.bp, state)
    ...
    def test_stale_snapshot_blocks_approval(self):
        ...
        self.fill_g1(state)
        projection = e.project(self.bp, state)
        snapshot = {"blueprint_version": "0.3", "projection_fingerprint": "older"}
        self.assertFalse(e.snapshot_is_fresh(snapshot, projection))
