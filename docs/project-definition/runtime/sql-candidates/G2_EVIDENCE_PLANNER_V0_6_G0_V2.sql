-- 4b4c / 2b2c — G2 Evidence Planner V0.6 — Basis + Inflight Guard + G0 V0.2
-- Date: 2026-09-13
-- STATUS: COMPILE/ROLLBACK CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- Requires: G0 resolver V0.2, final G2 helpers, recompute V0.3.

create or replace function public.plan_idea_evidence_context_candidate_v6(
  p_idea_id uuid,
...
begin
  ...
  v_context_plan:=public.plan_idea_creation_redesign_candidate_v2(
  ...
  v_recompute:=public.recompute_idea_evidence_context_candidate_v3(
...
revoke all on function public.plan_idea_evidence_context_candidate_v6(
  uuid,bigint,text[],boolean
) from public,anon,authenticated;
grant execute on function public.plan_idea_evidence_context_candidate_v6(
  uuid,bigint,text[],boolean
) to service_role;

-- V0.6 invariants:
-- - all V0.5 basis/inflight semantics preserved;
-- - G0 structural prerequisite uses basis-safe resolver V0.2;
-- - G2 recompute uses V0.3, which also depends only on G0 V0.2;
-- - no dependency on superseded G0 V0.1 or G2 recompute V0.2;
