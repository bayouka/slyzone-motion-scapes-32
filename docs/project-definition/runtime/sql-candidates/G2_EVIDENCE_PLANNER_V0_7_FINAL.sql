-- 4b4c / 2b2c — G2 Evidence Planner V0.7 FINAL CANDIDATE
-- Date: 2026-09-13
-- STATUS: COMPILE/ROLLBACK CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- Requires: G0 resolver V0.3 + G2 final helpers + recompute V0.4.

create or replace function public.plan_idea_evidence_context_candidate_v7(
  p_idea_id uuid,
...
begin
  ...
  v_context_plan:=public.plan_idea_creation_redesign_candidate_v3(
  ...
  v_recompute:=public.recompute_idea_evidence_context_candidate_v4(
...
revoke all on function public.plan_idea_evidence_context_candidate_v7(
  uuid,bigint,text[],boolean
) from public,anon,authenticated;
grant execute on function public.plan_idea_evidence_context_candidate_v7(
  uuid,bigint,text[],boolean
) to service_role;

-- Final candidate invariants:
-- - structural prerequisite comes from authoritative G0 resolver V0.3;
-- - recompute V0.4 always precedes planning;
...
-- - dependency readiness uses final authority-aware helper V3;
