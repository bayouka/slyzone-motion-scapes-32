-- 4b4c / 2b2c — G2 Evidence Planner V0.4 integrated candidate
-- Date: 2026-09-13
-- STATUS: COMPILE/ROLLBACK CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- Requires candidate resolver + recompute + planner helpers.

create or replace function public.plan_idea_evidence_context_candidate_v4(
  p_idea_id uuid,
...
begin
  ...
  -- Structural context is the first prerequisite, not a G2 research question.
  ...
  if coalesce(v_context_plan->>'status','')<>'RESOLVED' then
    return jsonb_build_object(
      ...
      'eligible_system_actions',case
        when v_context_plan->'eligible_system_action' is null or v_context_plan->'eligible_system_action'='null'::jsonb then '[]'::jsonb
        else jsonb_build_array(v_context_plan->'eligible_system_action') end,
  ...
  v_recompute:=public.recompute_idea_evidence_context_candidate_v1(
  ...
  for v_policy in select * from app_private.idea_g2_policy_v2() order by ordinal loop
    ...
    -- CONDITIONAL is blocking when applicable; ENHANCER is not.
  ...
  v_projection_fp:=md5(jsonb_build_object(
    ...
    'context_fingerprint',(select input_fingerprint from public.idea_requirement_states where idea_id=p_idea_id and requirement_id='SV.D04.CREATION_OR_REDESIGN'),
    'g2_fingerprints',v_fps,
  ...
  return jsonb_build_object(
    ...
    -- Business research never falls through to a generic human request.
...
revoke all on function public.plan_idea_evidence_context_candidate_v4(uuid,bigint,text[],boolean)
from public,anon,authenticated;
grant execute on function public.plan_idea_evidence_context_candidate_v4(uuid,bigint,text[],boolean)
to service_role;

-- V0.4 corrections:
-- - context resolver is a formal prerequisite;
-- - recompute always precedes planning;
-- - CONDITIONAL applicable Requirements block until satisfied/accepted according to policy;
-- - ENHANCER never blocks by itself;
-- - only the structural context resolver may return a targeted human action;
-- - G2 research itself remains system-first with no generic human fallback.
