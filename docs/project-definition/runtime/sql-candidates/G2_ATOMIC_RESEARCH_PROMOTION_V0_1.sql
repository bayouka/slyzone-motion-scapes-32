-- 4b4c / 2b2c — G2 atomic research promotion SQL candidate V0.1
-- Date: 2026-09-13
-- STATUS: DESIGN CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- This file intentionally lives outside supabase/migrations/.
-- Activation requires G1 runtime/E2E + Blueprint 0.5 full R0 replay + rollbacked SQL red-team.

-- -----------------------------------------------------------------------------
-- 1. Minimal system lineage for Sources
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2. G2 Action Run creation boundary
-- Keep create_action_run_v3 unchanged because G1 Foundation already depends on it.
-- V4 only widens permission_scope with SOURCE for explicit research paths.
-- -----------------------------------------------------------------------------
...
begin
  ...
  if coalesce(cardinality(p_target_requirement_ids),0)=0
     and coalesce(cardinality(p_target_artifact_keys),0)=0
    then raise exception 'ACTION_TARGET_REQUIRED';
  end if;
  ...
  if p_permission_scope ? 'allowed_mutation_kinds'
     and jsonb_typeof(p_permission_scope->'allowed_mutation_kinds')<>'array'
    then raise exception 'INVALID_PERMISSION_SCOPE';
  end if;
  ...
  loop
    ...
    if v_kind='SOURCE' and p_acquisition_path not in ('WEB','AUDIT','CONN')
      then raise exception 'SOURCE_PERMISSION_PATH_FORBIDDEN';
    end if;
  end loop;
...
-- -----------------------------------------------------------------------------
-- 3. Atomic promotion for research Actions
-- SOURCE proposals and INFORMATION_ITEM proposals live only inside the Action Run
-- until this transaction succeeds.
-- -----------------------------------------------------------------------------

create or replace function public.promote_research_action_result_v1(
...
begin
  ...
  -- No canonical Idea mutation is allowed between creation and promotion.
  ...
  -- Pass 1: Sources. They are created/reused/versioned before observations,
  -- but still inside the same transaction and before the single Idea revision bump.
  for v_mut in select value from jsonb_array_elements(v_run.proposed_mutations) loop
    ...
    if v_sensitivity not in ('public','internal','personal','sensitive')
      then raise exception 'INVALID_SENSITIVITY';
    end if;

    if v_run.acquisition_path='WEB' then
      if v_source_kind<>'url' or v_locator is null then raise exception 'WEB_SOURCE_INVALID'; end if;
    ...
    -- The executor must provide an already-normalized locator. SQL deliberately does not
    -- attempt to implement URL canonicalization or SSRF policy.
    select * into v_existing_source
    from public.idea_sources
    where idea_id=v_run.idea_id
      and source_kind=v_source_kind
      and locator is not distinct from v_locator
    ...
    if v_existing_source.id is null then
      insert into public.idea_sources(
        ...
        coalesce((v_mut->>'fetched_at')::timestamptz,now()),
      ...
      if v_existing_source.status in ('stale','failed') then
        -- A new successful fetch may refresh stale/failed source state, but never a superseded one.
        null;
      end if;

      v_old_rank := case v_existing_source.sensitivity
        when 'public' then 1 when 'internal' then 2 when 'personal' then 3 when 'sensitive' then 4 end;
      v_new_rank := case v_sensitivity
        when 'public' then 1 when 'internal' then 2 when 'personal' then 3 when 'sensitive' then 4 end;
      ...
      if v_existing_source.content_hash is distinct from v_content_hash then
        ...
        set content_hash=v_content_hash,
            ...
            fetched_at=coalesce((v_mut->>'fetched_at')::timestamptz,now()),
        ...
        update public.idea_sources
        set fetched_at=coalesce((v_mut->>'fetched_at')::timestamptz,fetched_at),
  ...
  -- Pass 2: observations / analyses / ledger proposals.
  for v_mut in select value from jsonb_array_elements(v_run.proposed_mutations) loop
    v_kind := v_mut->>'kind';
    if v_kind='SOURCE' then continue; end if;
    v_index := v_index+1;
    ...
    if v_kind='INFORMATION_ITEM' then
      ...
      if v_provenance not in (
        'SOURCE_EXTRACTED','CONNECTOR_EXTRACTED','WEB_RESEARCH','SYSTEM_CALCULATED',
        'AI_INFERRED','AI_RECOMMENDED'
      ) then raise exception 'INVALID_MACHINE_PROVENANCE'; end if;
      ...
      foreach v_level in array v_levels loop
        if v_level not in (
          'RAW_HUMAN','SOURCE_BACKED','OBSERVED','CALCULATED','WORKING_ASSUMPTION',
          'AI_RECOMMENDATION','ACCEPTED_AS_CURRENT','HUMAN_VALIDATED','HUMAN_DECISION',
          'EXPERT_SIGNOFF','ACCEPTED_UNKNOWN'
        ) then raise exception 'INVALID_RESOLUTION_LEVEL'; end if;
      end loop;
      ...
      if v_provenance='WEB_RESEARCH' then
        if v_run.acquisition_path<>'WEB' then raise exception 'PROVENANCE_PATH_MISMATCH'; end if;
        if v_source_id is null then raise exception 'SOURCE_REQUIRED_FOR_WEB_RESEARCH'; end if;
        if exists(select 1 from unnest(v_levels) x where x not in ('SOURCE_BACKED','OBSERVED'))
        ...
        if not exists(
          ...
          where s.id=v_source_id and s.idea_id=v_run.idea_id
            and s.source_kind='url' and s.status='ingested' and nullif(s.content_hash,'') is not null
        ) then raise exception 'WEB_SOURCE_NOT_CURRENT'; end if;
      elsif v_provenance='CONNECTOR_EXTRACTED' then
        if v_run.acquisition_path<>'CONN' then raise exception 'PROVENANCE_PATH_MISMATCH'; end if;
        if v_source_id is null then raise exception 'SOURCE_REQUIRED_FOR_CONNECTOR'; end if;
        if exists(select 1 from unnest(v_levels) x where x not in ('SOURCE_BACKED','OBSERVED'))
          then raise exception 'PROVENANCE_LEVEL_MISMATCH';
        end if;
      elsif v_provenance='SOURCE_EXTRACTED' then
        ...
        if exists(select 1 from unnest(v_levels) x where x<>'CALCULATED')
          then raise exception 'PROVENANCE_LEVEL_MISMATCH';
        end if;
      elsif v_provenance='AI_INFERRED' then
        if exists(select 1 from unnest(v_levels) x where x<>'WORKING_ASSUMPTION')
          then raise exception 'PROVENANCE_LEVEL_MISMATCH';
        end if;
      elsif v_provenance='AI_RECOMMENDED' then
        if exists(select 1 from unnest(v_levels) x where x<>'AI_RECOMMENDATION')
          then raise exception 'PROVENANCE_LEVEL_MISMATCH';
        end if;
      end if;
      ...
      insert into public.idea_information_items(
        ...
        v_provenance,v_source_id,nullif(v_mut->>'source_locator',''),v_confidence,'ACTIVE',
        coalesce(nullif(btrim(v_mut->>'sensitivity'),''),'internal'),now(),v_run.id,null,
      ...
      insert into public.idea_ledger_entries(
        ...
        coalesce(v_mut->'source_refs','[]'::jsonb),null,v_run.id
      );
  ...
  return jsonb_build_object(
    ...
    'sources_created',v_source_count,'information_items_created',v_info_count,
    'ledger_entries_created',v_ledger_count,'staled_information_items',v_staled_count,
    'idempotent',false
  );
end;
$function$;

revoke all on function public.promote_research_action_result_v1(uuid,text)
from public,anon,authenticated;
grant execute on function public.promote_research_action_result_v1(uuid,text) to service_role;

-- -----------------------------------------------------------------------------
-- Candidate review notes before migration promotion:
-- - run this against a disposable/rollbacked fixture first;
-- - validate null/invalid timestamp casts before final migration;
-- - validate existing duplicated active locators before considering a unique locator index;
-- - add explicit SQL red-team for concurrency and idempotent promotion;
-- - preserve create_action_run_v3/promote_action_result_v1 behavior for G1.
-- -----------------------------------------------------------------------------
