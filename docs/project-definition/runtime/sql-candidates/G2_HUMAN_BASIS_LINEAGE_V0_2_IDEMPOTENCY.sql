-- 4b4c / 2b2c — G2 direct-human basis lineage V0.2 — idempotency ordering
-- Date: 2026-09-14
-- STATUS: COMPILE/ROLLBACK CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- Supersedes G2_HUMAN_BASIS_LINEAGE_V0_1 for the future G2 bundle.
...
create or replace function public.apply_human_g2_information_candidate_v2(
  p_idea_id uuid,
...
begin
  ...
  -- Exact retry is recognized BEFORE current-state freshness checks. The original
  -- write itself increments engine_revision and marks the Requirement stale, so
  -- reversing this order would make network retries non-idempotent by construction.
  select * into v_existing
  from public.idea_information_items
  where idea_id=p_idea_id and idempotency_key=p_idempotency_key;

  if v_existing.id is not null then
    select rr.target_basis_fingerprint into v_existing_basis
    ...
    if nullif(btrim(v_existing_basis),'') is null then
      raise exception 'IDEMPOTENT_REF_BASIS_MISSING';
    end if;

    v_existing_fp:=md5(jsonb_build_object(
      ...
      'target_basis_fingerprint',v_existing_basis
    )::text);

    if v_existing.request_fingerprint is distinct from v_existing_fp then
...
revoke all on function public.apply_human_g2_information_candidate_v2(
  uuid,bigint,text,text,jsonb,text,text,text,uuid,uuid,text
) from public,anon;
grant execute on function public.apply_human_g2_information_candidate_v2(
  uuid,bigint,text,text,jsonb,text,text,text,uuid,uuid,text
) to authenticated,service_role;

-- V0.2 invariants:
-- - exact retries succeed even after the original write increments engine_revision;
-- - idempotency comparison is bound to the original target basis;
-- - a different payload with the same key is rejected;
-- - new writes still require current Idea revision and current Requirement materialization;
-- - direct G2 human support remains basis-scoped;
