-- 4b4c / 2b2c — G2 Evidence / Market recompute V0.6 — unified ref freshness
-- Date: 2026-09-14
-- STATUS: COMPILE/ROLLBACK CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- Requires:
--   - G0_CREATION_REDESIGN_RESOLVER_V0_4_RAW_SELECTION.sql
--   - G2_FINAL_HELPERS_V0_1_AUTHORITY.sql
--   - G2_HUMAN_BASIS_LINEAGE_V0_1.sql
--   - G2_REQUIREMENT_REF_FRESHNESS_V0_1.sql
...
create or replace function public.recompute_idea_evidence_context_candidate_v6(
  p_idea_id uuid,
...
begin
  ...
  v_context:=public.recompute_idea_creation_redesign_candidate_v4(
  ...
  for v_policy in select * from app_private.idea_g2_policy_v2() order by ordinal loop
    ...
    if v_policy.requirement_id='SV.D05.COMPETITOR_SET' then
      v_materiality:=app_private.idea_g2_competitive_materiality_v4(p_idea_id);
    ...
    if v_applicability='NOT_RELEVANT' then
      ...
      -- Unified freshness rule:
      --   machine ref => promoted Action Run targeted the current basis;
      --   direct human ref => ref.target_basis_fingerprint equals current basis.
      ...
      where rr.idea_id=p_idea_id
        ...
        and app_private.idea_g2_requirement_ref_current_v1(
          p_idea_id,ii.created_by_action_run_id,rr.target_basis_fingerprint,
          v_policy.requirement_id,v_basis
        );
      ...
      select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'target_basis_fingerprint',rr.target_basis_fingerprint
        ...
        where le.idea_id=p_idea_id
          ...
          and exists(
            ...
            where tr->>'requirement_id'=v_policy.requirement_id
              and (
                tr->>'basis_fingerprint' is null
                or tr->>'basis_fingerprint'=v_basis
              )
          )
      ) into v_conflict;
...
revoke all on function public.recompute_idea_evidence_context_candidate_v6(uuid,bigint)
from public,anon,authenticated;
grant execute on function public.recompute_idea_evidence_context_candidate_v6(uuid,bigint)
to service_role;

-- V0.6 invariants:
-- - selected RAW G0 basis semantics from V0.4 are preserved;
-- - machine and direct-human supporting refs use the same current-basis rule;
-- - legacy unscoped direct refs remain historical but cannot silently satisfy G2;
-- - target basis lineage is visible in resolution_refs;
-- - a parent basis change invalidates machine and human dependent support without deleting history;
-- - basis-scoped ACCEPTED_UNKNOWN remains valid only on the exact current basis;
-- - unscoped historical CONFLICT ledger entries remain conservative blockers until explicitly resolved;
-- - basis-scoped CONFLICT entries block only their matching current basis.