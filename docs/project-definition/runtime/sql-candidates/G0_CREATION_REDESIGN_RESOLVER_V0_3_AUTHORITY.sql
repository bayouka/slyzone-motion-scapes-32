-- 4b4c / 2b2c — Creation / Redesign context resolver V0.3 — Basis + authority
-- Date: 2026-09-13
-- STATUS: COMPILE/ROLLBACK CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- Supersedes candidate V0.2 for future G2 bundle only. Active G0/G1 runtime unchanged.

create or replace function public.recompute_idea_creation_redesign_candidate_v3(
  p_idea_id uuid,
...
revoke all on function public.recompute_idea_creation_redesign_candidate_v3(uuid,bigint)
from public,anon,authenticated;
grant execute on function public.recompute_idea_creation_redesign_candidate_v3(uuid,bigint)
to service_role;

create or replace function public.plan_idea_creation_redesign_candidate_v3(
  p_idea_id uuid,
  p_expected_engine_revision bigint,
...
declare
  ...
  v_satisfied boolean;
begin
  v_recompute:=public.recompute_idea_creation_redesign_candidate_v3(
...
revoke all on function public.plan_idea_creation_redesign_candidate_v3(uuid,bigint,boolean)
from public,anon,authenticated;
grant execute on function public.plan_idea_creation_redesign_candidate_v3(uuid,bigint,boolean)
to service_role;

-- Invariants:
-- - basis is RAW/Blueprint only; own result never changes own input fingerprint;
-- - fresh machine results must target the current basis;
-- - direct human corrections outrank machine extraction;
-- - contradictory values at the highest authority tier become CONFLICTED;
-- - EXISTING_SITE history cannot override the current structural context;
