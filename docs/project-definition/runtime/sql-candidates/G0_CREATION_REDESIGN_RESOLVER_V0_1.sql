-- 4b4c / 2b2c — Creation / Redesign context resolver V0.1
-- Date: 2026-09-13
-- STATUS: COMPILE/ROLLBACK CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- Reuses R3 Action Runs. Does not modify G1 planner or activate G2.

create or replace function public.recompute_idea_creation_redesign_candidate_v1(
  p_idea_id uuid,
...
begin
  ...
  if v_idea.blueprint_id<>'SITE_VITRINE' or v_idea.blueprint_status<>'active'
    then raise exception 'SITE_VITRINE_BLUEPRINT_NOT_ACTIVE';
  end if;

  select coalesce(
    max(s.content_hash) filter(where s.source_kind='human_raw' and s.status in ('registered','ingested')),
    md5(v_idea.original_text)
  ) into v_raw_hash
  from public.idea_sources s where s.idea_id=p_idea_id;
  ...
  if v_accepted_unknown then
    v_state:='ACCEPTED_UNKNOWN'; v_lock:='VALIDATED_CURRENT';
  elsif v_levels && array['ACCEPTED_AS_CURRENT','HUMAN_VALIDATED','HUMAN_DECISION']::text[] then
    v_state:='RESOLVED'; v_lock:='VALIDATED_CURRENT';
  elsif cardinality(v_levels)>0 then
  ...
  v_fp:=md5(jsonb_build_object(
    'blueprint',v_idea.blueprint_id||'@'||coalesce(v_idea.blueprint_version,''),
  ...
  insert into public.idea_requirement_states(
    ...
    p_idea_id,'SV.D04.CREATION_OR_REDESIGN',coalesce(v_idea.blueprint_version,'0.4'),
    'ACTIVE',v_state,'REQUIRED',v_lock,v_refs,v_fp,now(),null,1,v_idea.engine_revision,v_levels,true
  )
...
revoke all on function public.recompute_idea_creation_redesign_candidate_v1(uuid,bigint)
from public,anon,authenticated;
grant execute on function public.recompute_idea_creation_redesign_candidate_v1(uuid,bigint)
to service_role;

create or replace function public.plan_idea_creation_redesign_candidate_v1(
  p_idea_id uuid,
  p_expected_engine_revision bigint,
...
declare
  ...
  v_satisfied boolean;
begin
  v_recompute:=public.recompute_idea_creation_redesign_candidate_v1(
  ...
  select * into v_raw_source from public.idea_sources
  ...
  -- A successful fresh RAW run may deliberately return AMBIGUOUS with no canonical mutation.
...
revoke all on function public.plan_idea_creation_redesign_candidate_v1(uuid,bigint,boolean)
from public,anon,authenticated;
grant execute on function public.plan_idea_creation_redesign_candidate_v1(uuid,bigint,boolean)
to service_role;

-- Worker contract candidate for RAW extraction:
-- output schema: {classification: 'creation'|'redesign'|'AMBIGUOUS', support_text: string}
-- creation/redesign may promote only if support_text is an exact persisted RAW substring.
-- AMBIGUOUS completes the Action Run with result only and no proposed mutation.
