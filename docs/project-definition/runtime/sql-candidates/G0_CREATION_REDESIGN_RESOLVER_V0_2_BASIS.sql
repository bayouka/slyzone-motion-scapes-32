-- 4b4c / 2b2c — Creation / Redesign context resolver V0.2 — Basis-safe
-- Date: 2026-09-13
-- STATUS: COMPILE/ROLLBACK CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- Supersedes candidate V0.1 for future G2 bundle only. Active G0/G1 runtime unchanged.

create or replace function public.recompute_idea_creation_redesign_candidate_v2(
  p_idea_id uuid,
...
begin
  ...
  -- Basis = external/context input only. Own resolution is deliberately excluded.
  ...
  -- G0 structural context is not allowed to become silently sufficient through
  -- ACCEPTED_UNKNOWN. The ledger entry may remain history, but context stays unresolved.
  select exists(
  ...
  if v_levels && array['ACCEPTED_AS_CURRENT','HUMAN_VALIDATED','HUMAN_DECISION']::text[] then
  ...
  return jsonb_build_object(
    ...
    'input_fingerprint',v_basis,'raw_hash',v_raw_hash
  );
end;
$function$;

revoke all on function public.recompute_idea_creation_redesign_candidate_v2(uuid,bigint)
from public,anon,authenticated;
grant execute on function public.recompute_idea_creation_redesign_candidate_v2(uuid,bigint)
to service_role;

create or replace function public.plan_idea_creation_redesign_candidate_v2(
  p_idea_id uuid,
  p_expected_engine_revision bigint,
...
declare
  ...
  v_satisfied boolean;
begin
  v_recompute:=public.recompute_idea_creation_redesign_candidate_v2(
...
revoke all on function public.plan_idea_creation_redesign_candidate_v2(uuid,bigint,boolean)
from public,anon,authenticated;
grant execute on function public.plan_idea_creation_redesign_candidate_v2(uuid,bigint,boolean)
to service_role;

-- Candidate Worker RAW output:
-- {classification: 'creation'|'redesign'|'AMBIGUOUS', support_text: string}
-- creation/redesign promotion is allowed only if support_text is an exact persisted RAW substring.
-- AMBIGUOUS completes the run without canonical mutation; only then can planner expose human last-mile.
