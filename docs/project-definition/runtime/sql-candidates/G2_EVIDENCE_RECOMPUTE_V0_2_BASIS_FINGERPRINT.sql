-- 4b4c / 2b2c — G2 Evidence / Market recompute V0.2 — Basis Fingerprint
-- Date: 2026-09-13
-- STATUS: COMPILE/ROLLBACK CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- Requires candidate helpers from G0 resolver + G2 planner V0.1/V0.2/V0.3.
-- Blueprint 0.5 remains NON ACTIVE.

-- -----------------------------------------------------------------------------
-- 1. Resolution signature: child dependencies consume parent resolution, not
--    merely the parent's own input basis.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2. Source inventory basis. Research outputs are NOT injected into the basis
--    of the Requirement that launched that same research; only Requirements
--    explicitly depending on global evidence inventory use this helper.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 3. Materiality V3 removes the circular dependency on COMPETITOR_SET's own
--    current resolution. Existing competitor evidence may remain historical,
--    but it does not decide whether competitor evidence is materially required.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 4. Basis-scoped ACCEPTED_UNKNOWN for G2 candidate.
--    G1 active semantics stay unchanged. Future G2 acceptance must persist the
--    current basis_fingerprint in target_refs.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 5. Basis-aware recompute.
--    Order is deliberate:
--      basis -> fresh supporting results -> resolution -> parent signature.
-- -----------------------------------------------------------------------------

create or replace function public.recompute_idea_evidence_context_candidate_v2(
  p_idea_id uuid,
...
begin
  ...
  v_context:=public.recompute_idea_creation_redesign_candidate_v1(
  ...
  for v_policy in select * from app_private.idea_g2_policy_v2() order by ordinal loop
    ...
    -- Materiality is evaluated only after audit + evidence quality have been
    -- recomputed earlier in the ordered policy.
    ...
    -- -----------------------------------------------------------------------
    -- Build BASIS from context + parent RESOLUTION SIGNATURES only.
    -- Never include this Requirement's own state, levels or refs.
    -- -----------------------------------------------------------------------
    ...
    if v_applicability='NOT_RELEVANT' then
      ...
      -- Fresh support only. A machine-produced item counts only when the Action
      -- Run was promoted against THIS current basis fingerprint.
...
revoke all on function public.recompute_idea_evidence_context_candidate_v2(uuid,bigint)
from public,anon,authenticated;
grant execute on function public.recompute_idea_evidence_context_candidate_v2(uuid,bigint)
to service_role;

-- V0.2 invariants:
-- - own output never participates in own basis;
-- - parent resolution changes propagate through child basis fingerprints;
-- - machine results satisfy current Requirement only if promoted against current basis;
-- - stale/failed/superseded Sources never satisfy current Requirement;
-- - G2 ACCEPTED_UNKNOWN is scoped to exact basis_fingerprint;
-- - COMPETITOR_SET materiality is independent from COMPETITOR_SET's own result;
