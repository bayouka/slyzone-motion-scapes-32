-- 4b4c / 2b2c — G2 Evidence Planner V0.8 — selected RAW G0 V0.4
-- Date: 2026-09-13
-- STATUS: COMPILE/ROLLBACK CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- Requires: G0 resolver V0.4 + G2 final helpers + recompute V0.5.

create or replace function public.plan_idea_evidence_context_candidate_v8(
  p_idea_id uuid,
...
begin
  ...
  v_recompute:=public.recompute_idea_evidence_context_candidate_v5(
...
revoke all on function public.plan_idea_evidence_context_candidate_v8(
  uuid,bigint,text[],boolean
) from public,anon,authenticated;
grant execute on function public.plan_idea_evidence_context_candidate_v8(
  uuid,bigint,text[],boolean
) to service_role;

-- V0.8 invariants:
-- - all V0.7 planner semantics preserved;
-- - structural prerequisite uses selected-RAW basis G0 V0.4;
-- - recompute uses V0.5 so RAW changes propagate only when the consumed RAW Source changes;
