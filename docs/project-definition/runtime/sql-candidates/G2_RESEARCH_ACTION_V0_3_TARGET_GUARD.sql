-- 4b4c / 2b2c — G2 research Action Run target guard V0.3
-- Date: 2026-09-13
-- STATUS: ROLLBACK-TEST CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- Applies after the V0.2 candidate design. Replaces only create_action_run_v4.
...
begin
  ...
  -- V0.3 hardening: validate every target against the canonical Requirement cache now,
  -- before any external research can start. Promotion still revalidates the same fingerprints.
  foreach v_target in array coalesce(p_target_requirement_ids,array[]::text[]) loop
  ...
  values(
    ...
    jsonb_build_object(
      ...
      'engine_revision',v_idea.engine_revision,'research_boundary','v4-target-guard-v0.3'
    )
  );
...
-- Expected rollback tests:
-- - missing Requirement state -> TARGET_REQUIREMENT_STATE_REQUIRED;
-- - NOT_RELEVANT applicability -> TARGET_REQUIREMENT_NOT_ACTIVE;
-- - mismatched current fingerprint -> STALE_TARGET_REQUIREMENT;
-- - correct current fingerprint -> queued run;
-- - retry same idempotency key remains idempotent;
-- - create_action_run_v3 unchanged.
