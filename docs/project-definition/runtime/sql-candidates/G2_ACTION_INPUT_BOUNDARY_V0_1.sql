-- 4b4c / 2b2c — G2 Action Input Boundary V0.1
-- Date: 2026-09-15
-- STATUS: COMPILE/ROLLBACK CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
--
-- Purpose:
--   Give future evidence.advance executors a bounded, current, service-only input
--   snapshot without allowing the Worker to issue arbitrary service-role SELECTs.
--
-- Requires the G2 V0.4 candidate package, notably:
--   - app_private.idea_g2_run_targets_current_v2(uuid)
--   - target basis fingerprints on Action Runs
--   - Requirement state resolution_refs.

create or replace function public.get_g2_action_input_candidate_v1(
...
begin
  if nullif(btrim(p_current_input_fingerprint),'') is null then
    raise exception 'INPUT_FINGERPRINT_REQUIRED';
  end if;
  ...
  -- Provider/executor work is allowed only after the lifecycle start guard succeeded.
  ...
  if v_run.input_fingerprint is distinct from p_current_input_fingerprint then
    raise exception 'STALE_INPUT_FINGERPRINT';
  end if;
  ...
  if not app_private.idea_g2_run_targets_current_v2(v_run.id) then
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'criticality',rs.criticality_current,
  ...
  where rs.idea_id=v_run.idea_id
    and rs.requirement_id=any(v_run.target_requirement_ids);
  ...
  -- Only Information Items actually referenced by current materialized Requirement states
  -- are returned. Large value payloads are replaced with an explicit truncation marker.
  with ref_ids as (
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'id',ii.id,
    'semantic_key',ii.semantic_key,
    'item_type',ii.item_type,
    'value',case
      when octet_length(convert_to(ii.value_jsonb::text,'UTF8'))<=12000 then ii.value_jsonb
      else jsonb_build_object('_truncated',true,'semantic_key',ii.semantic_key)
    end,
    'provenance_type',ii.provenance_type,
    'confidence_class',ii.confidence_class,
    'sensitivity',ii.sensitivity,
  ...
  join public.idea_information_items ii
    on ii.id::text=r.item_id
   and ii.idea_id=v_run.idea_id
   and ii.state='ACTIVE';
  ...
  from (
    select s.id,s.created_at,jsonb_build_object(
      ...
      'locator',left(coalesce(s.locator,''),2048),
      'title',left(coalesce(s.title,''),400),
      'human_note',left(coalesce(s.human_note,''),1200),
      'content_hash',s.content_hash,
      'source_version',s.source_version,
      'fetched_at',s.fetched_at,
      'freshness_at',s.freshness_at,
      'status',s.status,
      'sensitivity',s.sensitivity
    ) as payload
    from public.idea_sources s
    where s.idea_id=v_run.idea_id
  ...
  if v_run.acquisition_path='RAW' then
    ...
    order by case when s.idempotency_key='system:initial-original-text' then 0 else 1 end,
             s.created_at asc,s.id::text asc
    limit 1;
    ...
    v_raw:=jsonb_build_object(
      ...
      'source_version',v_raw_source.source_version,
  ...
  return jsonb_build_object(
    'idea',jsonb_build_object(
      ...
      'title',left(v_idea.title,400),
      'current_description',left(v_idea.current_description,12000),
      'blueprint_id',v_idea.blueprint_id,
      'blueprint_version',v_idea.blueprint_version
    ),
    'action_run',jsonb_build_object(
      'id',v_run.id,
      ...
      'permission_scope',v_run.permission_scope
    ),
...
revoke all on function public.get_g2_action_input_candidate_v1(uuid,text)
from public,anon,authenticated;
grant execute on function public.get_g2_action_input_candidate_v1(uuid,text)
to service_role;

-- Invariants:
-- - no arbitrary table/query selector;
-- - service-role only;
-- - running Action Run only;
-- - exact Blueprint 0.5 / engine revision / input fingerprint / target basis required;
-- - current Requirement states only;
-- - current referenced Information Items only;
-- - bounded source metadata, never fetched page bodies;
-- - RAW original text only for RAW acquisition path and capped to 22k chars.
