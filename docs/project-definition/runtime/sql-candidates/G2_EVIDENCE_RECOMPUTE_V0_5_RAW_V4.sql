-- 4b4c / 2b2c — G2 Evidence / Market recompute V0.5 — selected RAW G0 V0.4
-- Date: 2026-09-13
-- STATUS: COMPILE/ROLLBACK CANDIDATE ONLY — DO NOT APPLY AS A MIGRATION.
-- Requires: G0 creation/redesign resolver V0.4 + G2_FINAL_HELPERS_V0_1_AUTHORITY.
...
create or replace function public.recompute_idea_evidence_context_candidate_v5(
  p_idea_id uuid,
...
revoke all on function public.recompute_idea_evidence_context_candidate_v5(uuid,bigint)
from public,anon,authenticated;
grant execute on function public.recompute_idea_evidence_context_candidate_v5(uuid,bigint)
to service_role;

-- Invariants inherited from V0.4 plus:
-- - structural G0 basis fingerprints the exact selected RAW Source (id/version/hash);
-- - non-selected RAW Sources do not create false staleness;
-- - selected RAW changes propagate through creation/redesign resolution signature into dependent G2 bases.
