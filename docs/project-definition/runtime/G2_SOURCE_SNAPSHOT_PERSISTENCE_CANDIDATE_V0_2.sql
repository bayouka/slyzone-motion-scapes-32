-- 4b4c / 2b2c — G2 Source Snapshot Persistence Candidate V0.2
-- STATUS: NON-ACTIVE CANDIDATE. DO NOT APPLY TO PRODUCTION WITHOUT VALIDATION REPORT.
-- Supersedes V0.1 candidate. Key corrections:
-- 1) snapshot immutability blocks UPDATE, not parent-cascade DELETE;
-- 2) exact duplicate source snapshot ids are rejected/canonicalized;
-- 3) dedicated SRC promotion wrapper enforces pinned source + sensitivity + exact V0.2 semantics.
...
-- Snapshot bodies are append-only. DELETE is intentionally not trigger-blocked so canonical
-- parent Source/Idea deletion can cascade and user deletion semantics remain intact.
create or replace function app_private.prevent_idea_source_snapshot_update_v1()
returns trigger
language plpgsql
set search_path=''
as $$
...
before update on public.idea_source_snapshots
for each row execute function app_private.prevent_idea_source_snapshot_update_v1();

-- Service-only bounded source ingestion. SHA-256 is computed over exact canonical extracted_text.
create or replace function public.commit_idea_source_snapshot_candidate_v2(
  p_source_id uuid,
...
begin
  if p_expected_source_version is null or p_expected_source_version<1 then raise exception 'EXPECTED_SOURCE_VERSION_REQUIRED'; end if;
  ...
  if p_extraction_metadata is null or jsonb_typeof(p_extraction_metadata)<>'object' then raise exception 'INVALID_EXTRACTION_METADATA'; end if;
  ...
  if v_source.source_kind<>'url' then raise exception 'SRC_V0_2_URL_REQUIRED'; end if;
  ...
  -- Same current body: return immutable snapshot idempotently, or backfill a metadata-only legacy Source.
  if v_source.content_hash=v_hash and v_source.status='ingested' then
    ...
    if v_snapshot.id is not null then
      ...
      return jsonb_build_object(
        'source_id',v_source.id,'snapshot_id',v_snapshot.id,'source_version',v_source.source_version,
        'engine_revision',v_idea.engine_revision,'content_changed',false,'snapshot_created',false,'idempotent',true
      );
    end if;
    ...
    return jsonb_build_object(
      'source_id',v_source.id,'snapshot_id',v_snapshot.id,'source_version',v_source.source_version,
      'engine_revision',v_revision,'content_changed',false,'snapshot_created',true,'idempotent',false
    );
  end if;
  ...
  set content_hash=v_hash,source_version=v_new_version,
      fetched_at=coalesce(p_fetched_at,now()),freshness_at=p_freshness_at,status='ingested',
      sensitivity=case when v_new_rank>v_old_rank then v_sensitivity else sensitivity end
  where id=v_source.id;
  ...
  return jsonb_build_object(
    ...
    'engine_revision',v_revision,'content_changed',v_changed,'snapshot_created',true,
    'staled_information_items',v_staled_count,'idempotent',false
  );
end;
$$;

revoke all on function public.commit_idea_source_snapshot_candidate_v2(uuid,integer,text,text,text,text,timestamptz,timestamptz,text,jsonb) from public, anon, authenticated;
grant execute on function public.commit_idea_source_snapshot_candidate_v2(uuid,integer,text,text,text,text,timestamptz,timestamptz,text,jsonb) to service_role;

create or replace function public.list_g2_src_ingestion_candidates_candidate_v2(
  p_idea_id uuid,
...
begin
  ...
  from (
    ...
    where s.idea_id=p_idea_id and s.source_kind='url'
      ...
      and not exists(
        ...
        where ss.source_id=s.id and ss.source_version=s.source_version and ss.content_hash=s.content_hash
      )
...
revoke all on function public.list_g2_src_ingestion_candidates_candidate_v2(uuid,bigint,integer) from public, anon, authenticated;
grant execute on function public.list_g2_src_ingestion_candidates_candidate_v2(uuid,bigint,integer) to service_role;

create or replace function app_private.idea_g2_src_has_current_snapshot_candidate_v2(p_idea_id uuid)
returns boolean
language sql
stable
security definer
...
revoke all on function app_private.idea_g2_src_has_current_snapshot_candidate_v2(uuid) from public, anon, authenticated;
grant execute on function app_private.idea_g2_src_has_current_snapshot_candidate_v2(uuid) to service_role;

-- Candidate planner predicate. Activation must explicitly replace/wire the current v1 predicate later.
...
as $$
  select case p_path
    ...
    when 'SRC' then app_private.idea_g2_src_has_current_snapshot_candidate_v2(p_idea_id)
...
create or replace function public.list_g2_src_snapshot_candidates_candidate_v2(
  p_idea_id uuid,
  p_expected_engine_revision bigint,
...
declare
  ...
  v_rows jsonb;
begin
  if p_requirement_id<>'SV.D03.PRIMARY_NEED' then raise exception 'SRC_V0_2_TARGET_UNSUPPORTED'; end if;
  select * into v_idea from public.ideas where id=p_idea_id;
  if v_idea.id is null then raise exception 'IDEA_NOT_FOUND'; end if;
  if v_idea.engine_revision<>p_expected_engine_revision then raise exception 'STALE_ENGINE'; end if;

  select coalesce(jsonb_agg(x.payload order by x.created_at,x.source_id,x.snapshot_id),'[]'::jsonb) into v_rows
  from (
    select ss.id snapshot_id,s.id source_id,ss.created_at,jsonb_build_object(
    ...
    where s.idea_id=p_idea_id and s.source_kind='url' and s.status='ingested'
      and s.sensitivity in ('public','internal') and ss.sensitivity in ('public','internal')
    order by s.created_at,s.id,ss.id
    limit v_limit
  ) x;
...
revoke all on function public.list_g2_src_snapshot_candidates_candidate_v2(uuid,bigint,text,integer) from public, anon, authenticated;
grant execute on function public.list_g2_src_snapshot_candidates_candidate_v2(uuid,bigint,text,integer) to service_role;

create or replace function public.create_g2_src_action_run_candidate_v2(
  p_idea_id uuid,
  p_expected_engine_revision bigint,
  p_requirement_id text,
...
begin
  ...
  select count(distinct x) into v_distinct_count from unnest(p_snapshot_ids) x;
  if v_distinct_count<>cardinality(p_snapshot_ids) then raise exception 'DUPLICATE_SRC_SNAPSHOT_ID'; end if;
  ...
  with requested as (select unnest(p_snapshot_ids) snapshot_id), valid as (
    select ss.id snapshot_id,s.id source_id,s.source_version,s.content_hash
    from requested r
  ...
  ) order by source_id,snapshot_id),'[]'::jsonb)
  ...
  v_created:=public.create_g2_system_action_run_candidate_v1(
    p_idea_id,p_expected_engine_revision,'EXTRACT_SOURCE','SRC',array[p_requirement_id],
    jsonb_build_object(p_requirement_id,p_target_fingerprint),array[]::text[],v_input_fp,p_projection_fingerprint,
    jsonb_build_object('allowed_mutation_kinds',jsonb_build_array('INFORMATION_ITEM')),
    'cloudflare-workers-ai','@cf/google/gemma-4-26b-a4b-it','g2-src-v2','g2-src-v2','source-extractor-0.2.0',p_idempotency_key
  );
  ...
  update public.idea_action_runs set input_refs=jsonb_build_object('source_snapshots',v_refs) where id=v_run_id;
  return v_created||jsonb_build_object('input_fingerprint',v_input_fp,'input_refs',jsonb_build_object('source_snapshots',v_refs));
end;
$$;

revoke all on function public.create_g2_src_action_run_candidate_v2(uuid,bigint,text,text,text,uuid[],text) from public, anon, authenticated;
grant execute on function public.create_g2_src_action_run_candidate_v2(uuid,bigint,text,text,text,uuid[],text) to service_role;

create or replace function public.get_g2_src_action_input_candidate_v2(
...
begin
  v_base:=public.get_g2_action_input_candidate_v2(p_action_run_id,p_current_input_fingerprint,p_expected_attempt);
  ...
  with refs as (
    select value ref from jsonb_array_elements(v_refs)
  ), valid as (
    select s.id source_id,ss.id snapshot_id,s.source_version,s.content_hash,s.locator,s.title,
           s.sensitivity,s.freshness_at,ss.content_type,left(ss.extracted_text,40000) extracted_text,
    ...
    where s.idea_id=v_run.idea_id and s.id=(r.ref->>'source_id')::uuid
      ...
      and s.source_version=(r.ref->>'source_version')::integer and ss.source_version=s.source_version
      and s.content_hash=r.ref->>'content_hash' and ss.content_hash=s.content_hash
      and s.sensitivity in ('public','internal') and ss.sensitivity in ('public','internal')
  )
  select count(*),coalesce(jsonb_agg(jsonb_build_object(
    ...
    'freshness_at',freshness_at,'content_type',content_type,'extracted_text',extracted_text,
...
revoke all on function public.get_g2_src_action_input_candidate_v2(uuid,text,integer) from public, anon, authenticated;
grant execute on function public.get_g2_src_action_input_candidate_v2(uuid,text,integer) to service_role;

-- Dedicated defense-in-depth promotion boundary for SRC V0.2.
create or replace function public.promote_g2_src_action_result_candidate_v1(
  p_action_run_id uuid,
  p_current_input_fingerprint text
)
returns jsonb
...
begin
  ...
  if v_run.promoted_at is not null then
    return public.promote_g2_system_action_result_candidate_v1(p_action_run_id,p_current_input_fingerprint);
  end if;

  if v_run.status<>'succeeded' then raise exception 'ACTION_RUN_NOT_PROMOTABLE'; end if;
  if v_run.acquisition_path<>'SRC' or v_run.action_type<>'EXTRACT_SOURCE' then raise exception 'SRC_PROMOTION_PATH_REQUIRED'; end if;
  if coalesce(cardinality(v_run.target_requirement_ids),0)<>1 or v_run.target_requirement_ids[1]<>'SV.D03.PRIMARY_NEED' then
    raise exception 'SRC_V0_2_TARGET_UNSUPPORTED';
  end if;
  if jsonb_typeof(v_run.proposed_mutations)<>'array' or jsonb_array_length(v_run.proposed_mutations)<>1 then
    raise exception 'SRC_V0_2_SINGLE_MUTATION_REQUIRED';
  end if;

  v_mut:=v_run.proposed_mutations->0;
  if v_mut->>'kind'<>'INFORMATION_ITEM' then raise exception 'SRC_V0_2_INFORMATION_ITEM_REQUIRED'; end if;
  if v_mut->>'target_requirement_id'<>'SV.D03.PRIMARY_NEED' then raise exception 'SRC_V0_2_TARGET_UNSUPPORTED'; end if;
  if v_mut->>'provenance_type'<>'SOURCE_EXTRACTED' then raise exception 'SRC_V0_2_PROVENANCE_REQUIRED'; end if;
  if v_mut->>'item_type'<>'DECISION_INPUT' then raise exception 'SRC_V0_2_ITEM_TYPE_REQUIRED'; end if;
  if coalesce(v_mut->>'confidence_class','')<>'DIRECT' then raise exception 'SRC_V0_2_DIRECT_CONFIDENCE_REQUIRED'; end if;
  if jsonb_typeof(coalesce(v_mut->'resolution_levels','[]'::jsonb))<>'array' then raise exception 'INVALID_RESOLUTION_LEVELS'; end if;
  v_levels:=array(select jsonb_array_elements_text(v_mut->'resolution_levels'));
  if v_levels<>array['SOURCE_BACKED']::text[] then raise exception 'SRC_V0_2_SOURCE_BACKED_ONLY'; end if;

  v_source_id:=nullif(v_mut->>'source_id','')::uuid;
  if v_source_id is null then raise exception 'SRC_V0_2_SOURCE_REQUIRED'; end if;

  v_refs:=coalesce(v_run.input_refs->'source_snapshots','[]'::jsonb);
  if jsonb_typeof(v_refs)<>'array' then raise exception 'SRC_INPUT_REFS_INVALID'; end if;

  for v_ref in select value from jsonb_array_elements(v_refs) loop
    if nullif(v_ref->>'source_id','')::uuid=v_source_id then
      select * into v_source from public.idea_sources s
      where s.id=v_source_id and s.idea_id=v_run.idea_id and s.source_kind='url' and s.status='ingested'
        and s.source_version=(v_ref->>'source_version')::integer and s.content_hash=v_ref->>'content_hash';
      if v_source.id is null then raise exception 'STALE_SOURCE_SNAPSHOT'; end if;

      select * into v_snapshot from public.idea_source_snapshots ss
      where ss.id=(v_ref->>'snapshot_id')::uuid and ss.source_id=v_source.id
        and ss.source_version=v_source.source_version and ss.content_hash=v_source.content_hash;
      if v_snapshot.id is null then raise exception 'STALE_SOURCE_SNAPSHOT'; end if;
      v_match:=true;
      exit;
    end if;
  end loop;

  if not v_match then raise exception 'SRC_SOURCE_NOT_PINNED'; end if;
  if v_source.sensitivity not in ('public','internal') or v_snapshot.sensitivity not in ('public','internal') then
    raise exception 'SRC_V0_2_SENSITIVITY_NOT_ALLOWED';
  end if;
  ...
  v_source_rank:=app_private.idea_sensitivity_rank_v1(v_source.sensitivity);
  if v_item_rank is null or v_source_rank is null then raise exception 'INVALID_SENSITIVITY'; end if;
  if v_item_rank<v_source_rank then raise exception 'SRC_SENSITIVITY_DOWNGRADE_FORBIDDEN'; end if;

  return public.promote_g2_system_action_result_candidate_v1(p_action_run_id,p_current_input_fingerprint);
end;
$$;

revoke all on function public.promote_g2_src_action_result_candidate_v1(uuid,text) from public, anon, authenticated;
grant execute on function public.promote_g2_src_action_result_candidate_v1(uuid,text) to service_role;

-- NOTE: this candidate intentionally does NOT alter active planner functions and does NOT
-- expose SRC at the Worker endpoint. Activation wiring is a separate post-validation change.
