...
as $$
declare
  v_actor uuid:=auth.uid();v_meeting public.meetings%rowtype;v_user uuid;v_role text;v_attendees uuid[];v_can_manage boolean;
begin
  ...
  v_can_manage:=v_meeting.created_by=v_actor
    ...
    or exists(select 1 from public.projects p where p.id=v_meeting.project_id and p.lead_user_id=v_actor);
  ...
  update public.meetings set title=btrim(p_title),starts_at=p_starts_at,ends_at=p_ends_at,
    video_room=nullif(btrim(coalesce(p_video_room,'')),''),visibility=p_visibility,agenda=btrim(coalesce(p_agenda,'')),
    live_notes=btrim(coalesce(p_live_notes,'')),summary=btrim(coalesce(p_summary,'')),status=p_status
