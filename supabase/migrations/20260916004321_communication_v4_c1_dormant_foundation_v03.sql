-- 2b2c Communication V4 — C1 dormant backend foundation candidate V0.3
-- SUPERSEDES V0.1 and V0.2. Candidate only: do not apply without transactional validation.
-- Authority:
-- - COMMUNICATION_V4_DATA_SECURITY_CONTRACT_V0_1.md
-- - COMMUNICATION_V4_C1_RED_TEAM_V0_1.md
-- - COMMUNICATION_V4_C1_RED_TEAM_V0_2.md
...
declare
  ...
  v_lineage_migration boolean := coalesce(current_setting('app.communication_v4_lineage_migration',true),'0')='1';
begin
  if tg_op='UPDATE' then
    if (new.parent_conversation_id is distinct from old.parent_conversation_id
        or new.source_message_id is distinct from old.source_message_id)
       and not v_lineage_migration then
...
drop policy if exists conversation_focus_members_select_v1 on public.conversation_focus_members;
...
-- Attachments are intentionally deferred until a safe storage preallocation
-- workflow exists. C1 must not advertise atomic opening attachments.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Existing V3 triggers materialize the readable audience. C1 then changes
  -- only this new V4 Discussion to the temporary V4 default.
...
-- ---------------------------------------------------------------------------
-- 9. Keep C1 truly dormant: NO API role can execute V4 mutations yet.
-- Tests may run inside controlled SQL transactions with a simulated user JWT.
-- C4 will explicitly grant authenticated EXECUTE after preview/E2E proof.
-- ---------------------------------------------------------------------------

revoke all on function public.create_discussion_v1(uuid,text,text,uuid,text,uuid[],uuid[]) from public, anon, authenticated, service_role;
revoke all on function public.set_discussion_focus_members_v1(uuid,uuid[]) from public, anon, authenticated, service_role;
revoke all on function public.set_conversation_follow_v1(uuid,text) from public, anon, authenticated, service_role;
revoke all on function public.resolve_discussion_v1(uuid) from public, anon, authenticated, service_role;
revoke all on function public.reopen_discussion_v1(uuid) from public, anon, authenticated, service_role;
revoke all on function public.mark_conversation_read_v4(uuid,uuid) from public, anon, authenticated, service_role;
