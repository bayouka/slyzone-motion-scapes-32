-- Deleted messages no longer expose original text or registered attachments to participants.
...
begin
  ...
  update public.messages
     set body='Message supprimé',
         subject=case when format='structured' then 'Message supprimé' else null end,
         deleted_at=now()
  ...
  select exists(
    ...
    where (
      ...
      x.segment2='messages' and x.conversation_id is not null and exists(
        ...
        where a.storage_path=p_name and m.deleted_at is null and c.id=x.conversation_id and c.workspace_id=x.workspace_id and app_private.can_access_conversation(c.id)
      )
    )
  );
$$;
