...
begin
  ...
  -- Room capacity is 6 total: initiator + at most 5 invitees.
  if cardinality(v_targets)>5 then raise exception 'CALL_TOO_MANY_INVITES'; end if;
...
begin
  ...
  if exists(
    ...
    where not exists(
      ...
      where wm.workspace_id=v_call.workspace_id
        and wm.user_id=x
        and wm.status='active'
    )
  ) then raise exception 'CALL_TARGET_UNAVAILABLE'; end if;

  -- Reserve seats for active participants, outstanding invitees, and the initiator
  -- if they have not joined yet. Maximum room capacity remains 6.
  ...
  if cardinality(v_targets) > v_available then
    raise exception 'CALL_CAPACITY_EXCEEDED';
  end if;
