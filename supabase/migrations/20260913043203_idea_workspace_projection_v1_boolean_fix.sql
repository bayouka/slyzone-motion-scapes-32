-- Forward-only correction for projection v1: boolean signals must never serialize as JSON null.
...
begin
  ...
  if not app_private.can_access_idea(p_idea_id) then raise exception 'FORBIDDEN' using errcode='42501'; end if;
  ...
  v_flags:=jsonb_build_object(
    ...
    'blueprint_mismatch',coalesce(v_idea.blueprint_status='mismatch',false),
